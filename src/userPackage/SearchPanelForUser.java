...
public class SearchPanelForUser extends JPanel {
	...
	public SearchPanelForUser(PersonModel pss) {
		ps=pss;
		...
		SearchPanelForUser.this.add(d1);
	...
	synchronized private void search(boolean searchByID) {
		...
		if (searchByID) {
			...
			for (FlightModel flt : flightList) {
				if(flt.status==flt.FLIGHT_CANCELLED || cd.getTimeInMillis()>flt.timeInMilli+flt.dateInMilli)continue;
